...
- **Zombies/** (new, P4, 2026-07-20): `AZombieCharacter` — deliberately does NOT reuse `UZSHealthComponent` (that component's wound/infection/treatment machinery is player-specific); just a flat replicated `CurrentHealth` seeded from `UZSZombieConfig::MaxHealth`, same `TakeDamage`-only-entry-point rule. `Server_MeleeAttack` applies `ZombieConfig->AttackDamageTypeClass` (falls back to `UZSDamageType_Bite`) via `UGameplayStatics::ApplyPointDamage`, self-validating range/cooldown. `AZombieAIController` wires `AIPerception` (sight + hearing, both `UAISenseConfig_*` radii read from `ZombieConfig` at `OnPossess`) and calls `RunBehaviorTree(ZombieConfig->BehaviorTree)` — **no Behavior Tree/Blackboard asset exists yet** (editor-authored content, not built blind); this is a graceful no-op until one is created and assigned. `UZSZombieConfig` covers speed/health/senses/damage/mesh/anim-class/BT per zombie type — zero C++ branches for a second type, per the multi-weapon-config rule. `UZSNoiseSystem::ReportNoise` (a thin wrapper over `UAISense_Hearing::ReportNoiseEvent`) is P4's "load-bearing" noise system — wired into weapon fire (`FireNoiseRadius` per weapon) and sprint start.
