...
> Design doc, approved 2026-08-01. Adoption in progress per the "Phased adoption plan" section near the bottom — Phase 0 (labels/board/protocol amendment) and Phase 1 (manual seed discovery pass) are being executed 2026-08-01; Phase 2 onward (anything that runs a build or creates a persistent scheduled task) is intentionally paused pending an explicit go-ahead. Extends `Docs/AsyncSessionProtocol.md`, doesn't replace it.
...
**Where this stands right now**: Phase 0 and Phase 1 are being executed in this session (see the rest of this doc's history/commits for what landed). Phase 2 onward — anything that actually runs a build or creates a persistent scheduled task — is intentionally paused for an explicit go-ahead, since that's the first point where something runs unattended against the real project instead of just being written down.
