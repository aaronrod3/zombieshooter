...
| M7 | Real mesh assembly, all 4 camera perspectives with real sockets/FOV, magazine spawn+reserve-hidden | Heavy editor/PIE | **In progress** — mesh assembly confirmed via component inspection (session 4); camera perspectives and magazine spawn blocked on a pending C++ recompile, see notes below. |
...
- Both fixes verified at the data/component level via MCP (`CurrentWeapon` now populates, `FirstPersonMesh`/`CharacterMesh0` have the config's `SKM_Manny_Simple` assigned, `ZSWeapon_0`'s receiver mesh is parented to `FirstPersonMesh`). The weapon-init fix is C++ and needs a Live Coding compile before ammo/magazine/camera-perspective behavior can be PIE-tested.
