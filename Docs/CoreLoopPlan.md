...
| M8 | `ABP_ZS_FirstPerson`/`ABP_ZS_ThirdPerson` AnimGraphs built to Guide 06/07's evaluation order | Heaviest — MCP's `BlueprintTools.read_graph_dsl`/`write_graph_dsl` confirmed unable to read/write AnimGraph pose content (risk spike, 2026-07-13); must use the lower-level `create_node`/`connect_pins` node API instead, unproven at AnimGraph scale — likely needs the dev's own hands for at least the harder parts (state machine/transition wiring) | Not started |
...
- **Verdict:** the convenient DSL text round-trip is off the table for building `ABP_ZS_FirstPerson`/`ABP_ZS_ThirdPerson`'s actual pose graphs. The structured node API (`create_node`/`connect_pins`/`add_node_pin`/`set_node_position`) is the only remaining MCP path and has real node-type awareness, but building a full state machine this way is unproven and likely slow/fragile — M8 should start with a small real test (two states + one transition on a throwaway AnimBP) before committing to the full build, and budget for needing the dev's own hands in the AnimGraph editor for at least the harder wiring.
