...
| M8 | `ABP_ZS_FirstPerson`/`ABP_ZS_ThirdPerson` AnimGraphs built to Guide 06/07's evaluation order | Heaviest — two risk spikes run 2026-07-13, both negative: (1) DSL round-trip can't read/write AnimGraph pose content; (2) headless AnimBlueprint *creation* itself hangs (likely an interactive skeleton-picker dialog with nothing to drive it). **Dev needs to manually create `ABP_ZS_FirstPerson`/`ABP_ZS_ThirdPerson` in the editor first** (target `SKM_Manny_Simple`); MCP can likely take over from there for `EventGraph` content, with `AnimGraph`/state-machine construction via `create_node`/`connect_pins` remaining unproven | Not started |
...
**Follow-up test attempted, hit a harder wall:** tried creating a throwaway test `AnimBlueprint` via `BlueprintTools.create` to test state-machine construction on it — **the call hung indefinitely (300s timeout, no asset actually created)**, most likely because `UAnimBlueprintFactory` normally requires an interactive skeleton-picker dialog that headless MCP calls can't drive. Confirmed the editor itself wasn't deadlocked (other calls succeeded immediately after) and no asset was left behind. **Practical conclusion: even creating the `ABP_ZS_FirstPerson`/`ABP_ZS_ThirdPerson` assets themselves will likely need the dev's own hands** (a quick manual step in the Content Browser, picking `SKM_Manny_Simple` as the target skeleton) — not just the harder state-machine/transition wiring. Once the assets exist, MCP can likely take over for `EventGraph` content (confirmed working) and for attempting `create_node`/`connect_pins` on the `AnimGraph` (unproven, but the node-type awareness is there) — full detail in `Docs/SessionHandoff.md`.

### M8 pre-work — FP arms mesh/skeleton correction (found while the dev was manually creating the AnimBPs)
Dev created `ABP_ZS_FirstPerson`/`ABP_ZS_ThirdPerson` and, for the FP arms, duplicated Infima's `SKM_FP_Manny_Simple` into a new `SKM_FirstPerson_Manny_Simple`, reassigning its skeleton to `SK_Mannequin` (Epic's stock skeleton). **Confirmed via MCP this is the wrong skeleton target** — every piece of Infima content in this project (including the already-working TP/FP body mesh) uses `SKEL_TFA_Mannequin`, a bundled duplicate of the same rig that Infima's own guide (`10_Custom_Character_And_Weapon_Import.md`) explicitly says to assign custom meshes onto directly, no retargeting needed. Bone hierarchies of `SK_Mannequin`/`SKEL_TFA_Mannequin` are identical (diffed, not assumed), but several sockets `AZSPlayerCharacter` depends on (`SOCKET_CameraFP` etc.) are defined at the *skeleton* level on `SKEL_TFA_Mannequin` specifically — the reassigned duplicate lost all of them. Duplicating the mesh at all is still the right call (matches Infima's own recommended workflow, and using the dedicated FP mesh instead of the full-body mesh is a real improvement over the current `DA_ZS_WeaponConfig_AssaultRifle.FP_Mesh` setup). Full detail, exact remaining steps, and the reasoning trail in `Docs/SessionHandoff.md` — not yet fixed, awaiting the dev's direction on MCP vs. manual.
