...
| M4 | 10 new `IA_*` assets (Fire/Aim/Reload/Crouch/Sprint/ToggleView/FireModeSwitch/Inspect/MagCheck/SwitchGrip) + `IMC_ZS_Default` mappings | Light editor/MCP | **Assets created, mappings status uncertain** — all 10 `IA_*` assets exist. Key mappings were written to `UInputMappingContext.mappings` and round-tripped correctly via MCP, but `V` (`IA_ToggleView`) didn't fire in PIE, and a second, separate `defaultKeyMappings.mappings` array was found containing its own independent (pre-existing, untouched-by-this-session) bindings. Which property the runtime/editor actually uses is **unconfirmed** — see `Docs/SessionHandoff.md`'s "Session-ending discovery," top priority for next session. |
...
| M6 | `DA_ZS_WeaponConfig_AssaultRifle` fully populated; `BP_ZS_PlayerCharacter` created and wired into `GameMode` | Heavy content population | **Done** — all fields populated, verified via readback. |
| M7 | Real mesh assembly, all 4 camera perspectives with real sockets/FOV, magazine spawn+reserve-hidden | Heavy editor/PIE | **In progress** — see notes below. Not yet compiled/tested. |
