...
- [x] [dev-editor] Stance selector's `bActiveValue` wired to a "crouched" Get node in-editor — *2026-07-20.* **Verification caveat, be honest about this:** MCP's own property/pin introspection still can't confirm the compiled `UZSAnimInstanceBase::bIsCrouched` (`find_node_types` still lists only the original 8 `Variables|ZS|Animation|*` getters, not one for `IsCrouched`, even after two Blueprint recompiles logged since) — so whatever this new Get node is actually bound to wasn't independently confirmable through available tooling, only that it's a valid zero-input-pin bool Get (not `ACharacter::IsCrouched()`, which would need a target pin). **Recommend a quick PIE crouch test** (crouch and confirm the pose actually changes) as the real, conclusive check — much more reliable here than another round of tool introspection.
- [x] [low priority] Assign a skeleton to the handful of wanted clips under `LyraAnims/Heroes/Mannequin/Animations/Actions/` (currently `Skeleton: None`, checked `MM_Pistol_Reload`/`MM_Rifle_Melee`) — only `Rifle_Melee` (Stage B melee swing) and the `HitReact` set (P3) are actually on our list; the rest of that folder is Lyra's out-of-scope action library.
- [x] Pistol animations/montages blocked on Infima releasing pistol content — rifle-only for now, not a blocker for Stage A (the shared locomotion + layered-pose architecture works per-weapon regardless).
