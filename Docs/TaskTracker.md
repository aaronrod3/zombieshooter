...
- [ ] [dev-editor] **⚠ CRITICAL PATH: finish retargeting the confirmed-useful blend spaces onto `SKEL_TFA_Mannequin`.** Confirmed direction (not a guess — verified via `get_asset_tags`): the project's real skeleton is `SKEL_TFA_Mannequin` and it stays; useful imported content gets retargeted onto it, no character skeleton swap. **`BS_UnequippedIdleWalkRun` is done** (confirmed fixed). **Still broken, same fix needed:** `BS_EquippedIdleWalkRun`, `BS_UnequippedCrouchWalk`, `AO_Ironsights_1D` (confirmed still pointing at the old missing path); `BS_EquippedCrouchWalking`/`AO_Ironsights_2D` presumed the same, not re-checked. **1 of ~6 done.** Full detail: GameDevPlan §5.1.
