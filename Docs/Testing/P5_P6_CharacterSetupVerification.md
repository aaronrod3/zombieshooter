...
The C++ half (`UZSAnimInstanceBase::bHasWeaponEquipped`) is done and reviewed. The graph-wiring half (a new `BlendListByBool` node between `LayeredBoneBlend_0` and the output slot, gated on that new bool) needs the new native property to actually exist in a running, compiled editor before it can be wired - it wasn't done in the same pass as the C++ change for exactly that reason. If Stage A still shows the rifle-pose bug, that's most likely why - check whether the graph wiring was actually completed before assuming something else is wrong.
