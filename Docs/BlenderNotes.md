...
**New curation folder, added 2026-08-29: `Content/LowPolyPackUsedMeshes/`.** The raw `Content/LowPolyWeapons/` pack turned out to be much larger than previously known (a 2026-08-29 pack audit found it covers Pistols/Shotguns/SMG/SniperRifle/Grenade/LMG/HandWeapon, not just AssaultRifle) and is already fully mirrored into `Content/ZS/Items/Weapons/Meshes/` in bulk — hundreds of unlabeled variant files per weapon type. This new folder is where the dev copies the *specific* mesh actually chosen for a given weapon once picked out of that bulk set, so the real, in-use asset for each weapon has one clear, easy-to-find home instead of being buried in a 900-file dump. Treat it as the destination once a candidate mesh from the mirrored pack is confirmed and selected — not a replacement for `Content/ZS/Items/Weapons/<Category>/`, which still holds the DA/BP/material for each weapon; this folder holds just the picked mesh itself.
