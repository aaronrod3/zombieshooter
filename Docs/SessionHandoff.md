...
**Every B1 task group (T1-T8) now has its C++ groundwork done, compiled, and automation-tested.** T1/T2/T5.0 closed out earlier this session (below); T3/T5 landed and got fixed up (`e064bc3`); this pass added T6 (container loot), T7 (death/respawn/sleep), and T8 (main menu/pause) in one clean rebuild+test pass (`00d3633`, first-try clean, no crashes). **Nothing left to write in C++ for B1 to be buildable end to end** - everything remaining is `WBP_ZS_*` widget construction plus a handful of explicitly-flagged open design questions and one infrastructure decision (Steam invite).

- **T6 (container loot) — commit `00d3633`**: `AZSContainerActor::Server_TakeItem` (GUID-exact per-item take, dupe-safe by construction - server RPCs execute serially, verified by `ZS.Inventory.ContainerTakeItemIsDupeSafe`), `Server_TakeAllItems` (the original "loot all" extracted/renamed, interact behavior unchanged), `Server_AddItemToContainer` (hand-placed items + test seeding), `OnContainerSlotsChanged` delegate (didn't exist before), and the real client→server RPC wrappers on `AZSPlayerCharacter` (`Server_TakeContainerItem`/`Server_TakeAllContainerItems`). **Open UX question, not decided**: should interacting with a container open a real loot screen instead of auto-looting (today's unchanged behavior)? The design session never mocked this up.
- **T7 (death/respawn/sleep) — commit `00d3633`**: new `FZSDeathInfo`/`UZSHealthComponent::LastDeathInfo`/`GetLastDeathInfo()` - "cause of death" didn't exist anywhere before this, captured on every hit in `Server_ApplyDamage`. `AZSPlayerCharacter::GetRespawnDelaySeconds()` (was `EditAnywhere`-only). `AZSGameState::GetSleepReadyCounts()` for a "2/4 ready" readout. Blackout (T7.3) needed no new code - already fully exposed. **Open question (OQ-B6-07)**: whether a fuller death-recap screen is wanted beyond plain cause-of-death.
- **T8 (main menu/pause) — commit `00d3633`**: new `UZSGameInstance` - the project had **no GameInstance subclass at all** before this. `HostGame`/`JoinGame` (plain travel, no online subsystem) and loading-screen delegates bound to `PreLoadMap`/`PostLoadMapWithWorld`. T8.2/T8.3 need no C++ (reuse `UZSUIManager` / stub button). **Steam invite deliberately not built** - `OnlineSubsystemSteam` isn't wired into `Build.cs`, an infrastructure decision for you to make. **Open question (OQ-B1-03)**: solo pause behaviour.
- **T3/T5 (HUD/inventory) — commit `e064bc3`**: moodle-stack aggregation, transparent-stat-preview contract, scoreboard/toast systems (T3); hotbar runtime-assignment, compartment filter, drag-drop payload (T5). Full detail in the prior version of this file / `B1_UI_UX.md`'s per-task entries.
- **11 new automation tests total this session, all passing** (hotbar assignment, compartment filtering, notifications, container take/take-all, death-info capture, sleep-ready counts). Only the same 5 pre-existing failures remain anywhere in the suite (list below, unchanged since before B1 started).
...
**Full click-by-click / per-widget steps** are in **`B1_UI_UX.md`'s "Manual setup steps" section** - every task group (T1-T8) now has its own entry listing every `WBP_ZS_*` widget still to build, which delegate/accessor each one binds to, and every open question flagged inline. A compiled visual checklist of the same list also exists as a private Claude artifact (not part of the repo) if useful while building.
