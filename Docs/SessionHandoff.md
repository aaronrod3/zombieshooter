...
**Rebuild first.** The 2026-08-02 accuracy-multiplier change (`GetAccuracySpreadMultiplier()`, see above) touched two headers plus a new `Tests/` file and is still uncompiled - full `Build.bat`, not Live Coding, then re-run the `ZS.` suite and confirm the new `ZS.Health.AccuracySpreadMultiplierScalesWithArmWound` test passes alongside the existing 5 pre-existing failures (still expected, unrelated).
