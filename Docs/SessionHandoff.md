...
- **One unresolved conflict, flag for the dev:** `OQ-B3-01` (save topology) got two different answers in the same batch — the file keeps the more detailed one (one continuously-overwritten world, no save-slot rollback) and flags the contradiction rather than guessing. Needs a one-line confirmation next time you're in.
- **One open ask for Claude's input, answered but not yet confirmed:** `OQ-B10-04` (disconnect handling) — the dev asked for help designing an anti-combat-log system; a proposal (safe-logout reusing the sleep-safety gate + a server-tracked in-combat flag) is drafted in `90_OpenQuestions.md`, needs a go/no-go.
