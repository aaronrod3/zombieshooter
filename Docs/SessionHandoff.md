...
**Rebuild first.** Two uncompiled 2026-08-02 changes stacked this session - the accuracy-multiplier change (`GetAccuracySpreadMultiplier()`) and the manifest-widget-logic-into-C++ pass (`UZSUserWidgetBase` getters, `GetWoundDisplayCondition`, `GetKeyLabelForHotbarIndex`, `UZSDragDropPayload::Make`) - touching `ZSHealthComponent.h/.cpp`, `ZSHealthConfig.h`, `ZSHealthTypes.h`, `ZSUserWidgetBase.h/.cpp`, `ZSPlayerCharacter.h/.cpp`, `ZSDragDropPayload.h/.cpp`, and `Tests/ZSAutomationTests.cpp`. Full `Build.bat`, not Live Coding, then re-run the `ZS.` suite and confirm both new tests pass (`ZS.Health.AccuracySpreadMultiplierScalesWithArmWound`, `ZS.Health.WoundDisplayConditionIsZoneAwareAndPrioritized`) alongside the existing 5 pre-existing failures (still expected, unrelated).
