...
- **T1 (Input-mode switching) — DONE**, solo PT1 passed clean in PIE 2026-08-01 (dev-confirmed): `UZSUIManager` modal stack + hard-gated `HandleAttack`/`IsCursorFacingActive`, `IA_UISelect`/`IA_UICancel`/`IA_UINavigate`/`IMC_ZS_UI` all correctly mapped. T2+ are unblocked. Still open, not blocking: PT1's 2-client half (this doc's own carried-forward B0 2-client baseline sweep - fire/reload/aim/sprint/crouch/hotbar/melee/loot/drop from both clients - is folded into this exact checkpoint per `B1_UI_UX.md`'s Exit criteria note, deliberately deferred as its own session rather than rushed).
...
**T1 is closed.** Everything through `de612a5` compiles, passes automation, and passed solo PT1 in PIE - no rebuild needed to catch up. From here: **check `DA_Bag.uasset`'s `EquipSlot`** for the Hip→Duffle content-risk noted above (not yet done), create T2's `DA_ZS_UIStyle_Default` data asset, and decide on `HotbarSlots`' rewiring scope when ready to continue T5.0 (see `B1_UI_UX.md`'s Manual setup steps for exact steps on all of these). PT1's 2-client sweep and T3 (HUD, depends on T2) are the two bigger pieces still ahead. Read `Docs/Planning/B1_UIDesignSession_2026-07-30.md` first if picking this up cold.
