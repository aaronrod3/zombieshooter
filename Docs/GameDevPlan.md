...
- **Still no melee `UZSWeaponConfig` instance actually authored** - blocked on deciding how a melee weapon attaches/displays (`TP_Mesh` is a full-body skeletal swap for the gun-holding pose; a one-handed bat/pipe probably wants a simpler held-prop attachment instead), not resolved yet.
