...
- **Still not built**: the actual `PrimaryHand`/`SecondaryHand`/hotbar loadout slots (today "equipped" just means `CurrentWeapon`, which auto-equips at `BeginPlay` from `StartingWeaponConfig` and has no in-game unequip/switch path yet), and real per-weapon melee stats on `UZSWeaponConfig` (damage/range/interval fields, montage) for whenever a melee weapon actually gets authored — `Melee`-typed configs will silently fall back to bare-fist stats until those fields exist.
