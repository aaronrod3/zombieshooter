...
- **Infection as a visible, urgent countdown, not a guessing game** — ⚑ **REVISED 2026-07-26, dev decision (rescope pass).** PZ's own "is this a cold or am I dead?" ambiguity was the original inspiration and shipped that way through B0's initial design, but the dev's explicit call is to **plainly show the player when they've been bitten and when they're infected** — clear feedback, not a deliberately unreadable moodle. The tension moves from "can I even tell what's happening to me" to "I know exactly what's happening and I have to act." **Emergency amputation** of a bitten/infected limb can stop a fatal infection before it completes, at the cost of permanent capability loss (Notes §1/§9: "second chances... such as reviving, cutting off infected body parts") — that escape valve is unchanged, only the UI-legibility question changed. Any doc still describing this as "deliberately ambiguous" (`CR-06`/`OQ-B0-07`/B0-T6.3/B1-T3.3 in `Docs/Beta/`) is describing the superseded design; see the Beta plan's updated Contradiction Register.
...
| Hand-built mega-map (§10.1) | **REPLACE** ⚑ *revised 2026-07-26* | A fictional Adirondacks-modeled county, **larger than the original ~1×1 km proposal** (dev call, in support of vehicles landing later and 4+ players) — but built **in phases**, not blocked out all at once: one fully-realized dense district first (to the real quality bar, playtested), then expand outward district by district as a continuous track rather than a single monolithic content phase. World Partition. |
...
| MP: dedicated servers, big counts (§19) | **SIMPLIFY** ⚑ *revised 2026-07-26* | **4+ player co-op**, listen-server as the primary/default mode; an optional paid dedicated-server hosting path for groups who want one, added later — not required for Stage 1 or for the internal beta gate. Raises B8's network/perf budget math versus the original 2–4 assumption; re-baseline before locking numbers. |
