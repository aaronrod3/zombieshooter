...
>
> **⚑ PIVOT NOTICE, 2026-08-29 — persistent world / physical hub, two days later.** The "raid" half of §1's one-liner below is now superseded: there is no more discrete raid session to reach an extraction point in or die during. The **hub is a physical, walkable, persistent location — Grayback Lodge**, a mountain hunting-cabin compound — sitting inside **one continuously-simulated, never-reloaded quarantine zone**. Players drive or walk freely between compound and zone, individually, with no party lockstep or load boundary. **What's unchanged**: permadeath itself (character loses skills/XP/everything carried on death; dropped loot stays exactly where it fell for a future character to recover; the hub's stash/vendor rep/currency/upgrades are untouched by any death) and the whole vendor/contract/economy layer. **What's new**: a zombie spawn-director + horde-migration system replaces per-raid reseed (zombies refill off-screen and roam in migrating hordes; loot is finite and does not replenish once taken), and a first slice of drivable vehicles (basic transport only, full fuel/damage/combat depth stays with `BV`'s own deferred pass). Full detail: `Docs/Beta/00_MasterPlan.md` CR-14, `Docs/Beta/BH_HubHideoutEconomy.md` and `Docs/Beta/BR_RaidLifecycleExtraction.md`'s own pivot notices. **§1's prose below is not yet rewritten line-by-line for this second pivot** — read the one-liner's "a raid ends by reaching an extraction point... or by dying" clause as superseded by this notice, and reasoning point 2 ("sessions have a shape") as largely retired; everything else in §1 (permadeath's meaning, the hub's persistence role, the moodle/skill reshaping) still holds.
