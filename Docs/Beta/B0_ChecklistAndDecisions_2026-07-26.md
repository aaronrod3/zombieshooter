...
**Current status: 14 tests total.** 10 have actually been run (9 pass, 1 fails on a real content-gap finding). The 4 newest — 2 latent (time-based) plus 2 new `SecondaryWeapon`-lifecycle tests, all written 2026-07-29 — are written and traced carefully against the current source but have never been compiled or run. Say the word for a test-automation session (present-session, dev-triggered only, see `CLAUDE.md`) to get their first real run.
