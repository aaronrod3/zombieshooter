...
**Current status: 20 tests total.** 10 have actually been run (9 pass, 1 fails on a real content-gap finding). The 10 newest, all written 2026-07-29, have never been compiled or run — 2 latent (time-based), 2 `SecondaryWeapon`-lifecycle tests, and 6 more added alongside two rounds of real bug fixes found by code review (see below). Since a C++ file compiles as one unit, the whole file's build status is unknown until the next rebuild, not just the newest tests' — say the word for a test-automation session (present-session, dev-triggered only, see `CLAUDE.md`) to get a real run.
...
| `ZS.Loadout.SecondaryWeaponEquipAndUnequip` | `AZSPlayerCharacter::SecondaryWeapon`'s spawn/attach/destroy lifecycle in isolation, using an in-memory `NewObject<UZSWeaponConfig>()` rather than a named content asset | 🔧 Written 2026-07-29, **never run** |
| `ZS.Loadout.SecondaryWeaponDurabilityWriteback` | Equip → consume durability → unequip (writeback) → re-equip → confirm it resumes at the written-back value, not full | 🔧 Written 2026-07-29, **never run** — caught a real bug while writing it, see below |
| `ZS.Combat.ZombieDeathWhileDownedClearsDownedFlag` | `AZombieCharacter::Die()` now clears `bIsDowned` - a downed zombie killed via ranged fire used to stay flagged downed forever | 🔧 Written 2026-07-29, **never run** — protects a real bug fix, see below |
| `ZS.Survival.SleepReadyClearedOnDeath` | `HandleDeath()` now cancels sleep-readiness - a dead player's stale ready flag used to keep counting toward the party's sleep aggregation | 🔧 Written 2026-07-29, **never run** — protects a real bug fix, see below |
| `ZS.Loadout.SecondaryWeaponRackFirearmClearsJam` | `CanRackFirearm`/`Server_StartRackFirearm` now consider `SecondaryWeapon` too - a jammed offhand weapon used to have no way to ever clear the jam | 🔧 Written 2026-07-29, **never run** — protects a real bug fix, see below |
| `ZS.Health.DeathWritesBackDurabilityAndDestroysWeapons` | Death now writes back weapon durability and destroys `CurrentWeapon`/`SecondaryWeapon` before dropping loot - both used to be skipped entirely | 🔧 Written 2026-07-29, **never run** — protects a real bug fix, see below |
| `ZS.Health.FractureUpgradeClearsBleedFlag` | `Server_ApplyDamage`'s bleed-flag logic now gates on the zone's resolved `WoundType`, not the incoming hit's - a lower-severity hit onto an already-Fractured zone used to incorrectly set `bBleeding=true` | 🔧 Written 2026-07-29, **never run** — protects a real bug fix, see below |
| `ZS.Inventory.StoreInBagRejectsEquippedInstance` | `Server_StoreInBag` now rejects storing an instance equipped to `EquippedBack`/`EquippedHip` - used to silently orphan the gear slot's GUID. Partial fix only, see below | 🔧 Written 2026-07-29, **never run** — protects a real bug fix, see below |

**Four more real bugs found and fixed while code-reviewing the rest of B0, 2026-07-29** (same motivation as the durability-writeback fix above - no PIE access, so auditing instead of stalling): (1) `Server_HandleDeathLootAndZombie` never wrote back live weapon durability or destroyed the equipped weapon actors before dropping loot - a permanent actor leak plus stale-durability loot on every death with a weapon equipped. (2) `AZombieCharacter::Die()` never reset `bIsDowned` - a zombie killed while downed stayed flagged downed on its corpse forever. (3) `AZSGameState::UpdateSleepRequestState`'s aggregation never accounted for a dead/blacked-out player, and neither `HandleDeath()` nor `EnterBlackout()` cancelled sleep-readiness - a party's sleep/time-skip could succeed off an incapacitated teammate's stale ready flag. (4) `CanRackFirearm`/`Server_StartRackFirearm` were hard-coded to `CurrentWeapon` only - a jammed offhand weapon had no way to ever clear the jam, a permanent soft-lock. All four fixed by reusing existing functions already established elsewhere in the codebase (no new abstractions) - full detail in `B0_Stabilization.md`'s T9.1/T10.1/T10.4/T4.10 rows.

**Two more, lower-severity, found in the same original research pass but deferred to a follow-up round, 2026-07-29**: (5) the bleed-flag bug above - gated on the incoming hit's `WoundType`, not the zone's resolved one, so a lower-severity hit onto an already-Fractured zone could set/leave `bBleeding=true` on a zone that never actually drains from it. Fixed. (6) `Server_StoreInBag` never guarded against storing a currently-equipped instance - **only partially fixed**: the `EquippedBack`/`EquippedHip` half lives on `UZSInventoryComponent` itself and needed no new cross-component reach, so it's fixed; the `HotbarSlots`/`SecondaryHandInstanceId` half lives on `AZSPlayerCharacter` and would need either the character validating before calling in, or a new cross-component query - a real design call, left open on purpose rather than guessed at. Full detail in `B0_Stabilization.md`'s T5.3/T2.9 rows.

**None of this has been compiled.**

**Real bug found and fixed while writing the newest batch, 2026-07-29**: `Server_EquipToSecondaryHand_Implementation` was calling `EquipSecondaryWeapon(Config)` without ever seeding durability/condition from the carried instance — unlike the primary hand's hotbar-equip completion handler, which explicitly calls `CurrentWeapon->SeedDurabilityFromInstance(...)` after equipping. Every offhand weapon silently reset to full durability on every equip instead of resuming where it left off — the exact bug class Checkpoint B (`ZS.Weapons.DurabilityPersistence`) exists to catch, just on the secondary-hand path, which had zero coverage until now. Fixed in `Server_EquipToSecondaryHand_Implementation` (`ZSPlayerCharacter.cpp`); `ZS.Loadout.SecondaryWeaponDurabilityWriteback` now directly protects it, but neither the fix nor the test has been compiled yet.
