...
**Current status: 18 tests total.** 10 have actually been run (9 pass, 1 fails on a real content-gap finding). The 8 newest, all written 2026-07-29, have never been compiled or run — 2 latent (time-based), 2 `SecondaryWeapon`-lifecycle tests, and 4 more added alongside a round of real bug fixes found by code review (see below). Since a C++ file compiles as one unit, the whole file's build status is unknown until the next rebuild, not just the newest tests' — say the word for a test-automation session (present-session, dev-triggered only, see `CLAUDE.md`) to get a real run.
