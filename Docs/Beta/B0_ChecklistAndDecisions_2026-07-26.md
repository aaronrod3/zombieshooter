...
- **"Always Torso, no matter which angle I approach from" — ✅ fixed 2026-07-30, needs a rebuild and retest.** Root cause: `AZombieCharacter::Server_MeleeAttack` (`ZombieCharacter.cpp`) determined the hit zone with a straight horizontal line-trace at a **fixed** `+40` Z-offset regardless of approach direction, which lands on Torso-region bones essentially every time. Fixed with a weighted random zone roll (`UZSZombieConfig::HeadBiteChance`/`ArmsBiteChance`/`LegsBiteChance`, defaults 10%/20%/15%, Torso the implicit ~55% remainder), mirroring the player's own headshot-weighting precedent (`Server_Fire_Implementation`'s `Hit.BoneName` override) — same minimal-diff approach, a chance-roll override, not real geometry. New test: `ZS.Combat.ZombieBiteZoneWeightedRoll`. **Retune the three chances in `TuningReference.md`/the Details panel if the split doesn't feel right** — these were a reasonable starting default, not dev-tested numbers.
...
1. **Zone variance.** Unblocked as of 2026-07-30 — rebuild first (see the fixed finding above).
   - **Pass:** the zone showing a non-`None` `WoundType` varies across attempts, not always Torso. Expect roughly the configured odds (10% Head / 20% Arms / 15% Legs / ~55% Torso by default) over enough bites, not a strict per-hit guarantee.
