...
### CR-14 — Persistent world / physical hub pivot ✅ RESOLVED 2026-08-29 (dev-confirmed) — reverses CR-13's session-boundary half two days after CR-13 landed

| | |
|---|---|
| **What changed** | Two days after CR-13 replaced the persistent-open-world model with a hub-and-raid extraction loop, the dev reversed the *session-boundary* half of that loop: the hub becomes a **physical, walkable, persistent location** — a mountain hunting-cabin compound named **Grayback Lodge** — sitting inside (or directly adjoining) **one continuously-simulated, persistent quarantine zone**, not a menu-driven screen flow reached by "entering a raid." There is no more discrete "raid" as a bounded, reseeded, level-reloaded session. Players drive or walk between the compound and the zone freely, individually, at any time — no party lockstep, no load screen. |
| **What CR-13 said, now superseded** | "A raid ends by reaching an extraction point (loot banked) or by dying," "each raid entry reseeds loot-container contents, zombie density/placement, and hostile placement," "entering the zone reloads the level" (`OQ-BR-03`), "a player who dies spectates the rest of the party until the raid ends" (`OQ-BR-01`), "no hub level content at all" (`OQ-BH-01`). **All five of these are reversed by this entry** — see `90_OpenQuestions.md`'s `OQ-BH-01`/`OQ-BR-01`/`OQ-BR-03` entries, now marked SUPERSEDED with links here, plus four new resolved questions (`OQ-BH-04`, `OQ-BR-04`..`06`) this pivot required. |
| **What's unchanged from CR-13** | The stash/vendor/contract economy, per-player stash on `AZSPlayerState`, and — critically — **permadeath itself is unchanged in substance**: a character still permanently loses its skills/XP and everything it was carrying on death. What changes is only the *container* around that loss: there's no "raid" to have died *during*, the character just dies wherever it is in the persistent zone, its dropped gear stays exactly where it fell (already-built `Server_DropAllItems`/`AZSWorldItemActor` behavior, now serving a persistent world instead of a reseeded one), and any future character can walk or drive back to that spot and recover it. Hub-side stash/vendor rep/currency/hideout upgrades remain untouched by any character's death, same as CR-13 already established. |
| **Dev-confirmed specifics (this session)** | (1) **Theme**: Grayback Lodge — a converted hunting lodge/outfitter compound on a timbered ridge, repurposed as a mercenary forward base (armory built from the old gun racks, a Motor Barn for stored/recovered vehicles, a root-cellar-style stash vault, a ridge lookout tower). (2) **Zombie persistence**: no level-reload reseed — instead a spawn-director periodically fills zombies back into map areas the players aren't currently looking at (avoids visible pop-in), plus **hordes periodically migrate through the map** as a roaming threat, independent of player position. (3) **Loot persistence**: the world seeds with a substantial amount of items up front, but **containers do not replenish once emptied** — a finite, fully consumable loot economy, deliberately different from the zombie side. (4) **Death**: character resets (skills/XP gone, matches CR-13's existing permadeath model) and everything it was carrying stays at the death location for any future character to recover; the hub/camp area and everything in it is untouched and remains accessible to a brand-new character. (5) **Multiplayer**: players move fully independently — any subset of the group can be in the zone while others stay at the compound, no shared session boundary. (6) **Vehicles**: promoted out of `BV`'s "own scoping pass, unscheduled" holding pattern (`CR-02`) for a first slice now — real drivable transport (movement + enter/exit seats) in and out of the compound, explicitly **without** `BV`'s fuller fuel/damage/combat depth, which stays deferred to its own pass. |
| **Assumption flagged, not yet dev-confirmed** | With no more discrete "raid" to extract *from*, the extraction-point ceremony (`AZSExtractionPointActor`, `Server_RequestExtraction`'s ceremonial banking moment) likely has no reason to exist as a separate mechanic — the natural replacement is that carried loot simply becomes safe once physically deposited into the stash at the compound (an ordinary container-deposit interaction, not a special "extract" action). This is my inference from the shape of the other five answers, not something asked directly — flag it for confirmation before removing `AZSExtractionPointActor` or `Server_RequestExtraction`. |
| **What carries over essentially unchanged** | Everything CR-13 already carried over (health/wounds/combat/weapons/inventory/zombie AI/B1 UI) — this pivot only touches the world-structure and session-boundary layer around them, same as CR-13 didn't touch the mechanical layer under it. |
| **Genuinely new systems, no prior phase home** | Off-screen zombie spawn-director + horde-migration system (reframes most of `BR`'s original "raid reseed" task line — see `BR_RaidLifecycleExtraction.md`'s new pivot notice), a basic drivable-vehicle pawn/actor (a first slice of `BV`, not the full phase), a physical/walkable version of `BH`'s stash/vendor/contract interactables (container-style interact-to-open, not menu-driven navigation). |
| **Not yet touched** | **No C++ has been changed for this pivot** — `AZSGameState::IsRaidOver`/`Server_CheckRaidEndAndReset`, the spectate-until-raid-ends flow, and `Server_RequestExtraction` are all still exactly as CR-13 left them, now understood to need rework or removal once this reaches implementation. This entry is a design-record-only pass, matching the project's own "plan before big work" discipline — same treatment CR-13 itself got before any code followed. |
| **Full detail** | `Docs/GameDevPlan.md` §1's new pivot notice, `Docs/Beta/BH_HubHideoutEconomy.md`'s rewritten hub description, `Docs/Beta/BR_RaidLifecycleExtraction.md`'s new pivot notice, `90_OpenQuestions.md`'s `OQ-BH-01`/`OQ-BH-04`/`OQ-BR-01`/`OQ-BR-03`/`OQ-BR-04`..`06`. |
...
| **BH** | *(new, CR-13, scoped 2026-08-27 — see `BH_HubHideoutEconomy.md`)* Hub/Hideout & Economy — stash, vendor NPCs, currency, contract-giving, loadout prep | **L** *(estimate, no historical basis to size against)* | — *(placeholder)* | `[INTERNAL]` |
| **BR** | *(new, CR-13, scoped 2026-08-27 — see `BR_RaidLifecycleExtraction.md`)* Raid Lifecycle & Extraction — raid start/reseed pass, extraction points, permadeath save flow | **L** *(estimate, no historical basis to size against)* | — *(placeholder)* | `[INTERNAL]` |
...
| **BF** | *(new, CR-13, scoped 2026-08-27 — see `BF_HumanHostileFaction.md`)* Human Hostile AI Faction — promoted from the old "post-v1 hostile roamers" deferral to a core system; sibling to `AZombieCharacter`, reuses the shared combat/damage pipeline | **M** *(estimate — has a real precedent, GameDevPlan.md Decision 5's own "cheap AI reuse" framing)* | — *(placeholder)* | `[PUBLIC]` |
...
**Note on `BH`/`BR`/`BF` (CR-13, added 2026-08-27, scoped 2026-08-27):** each now has a real task-breakdown file (`BH_HubHideoutEconomy.md`, `BR_RaidLifecycleExtraction.md`, `BF_HumanHostileFaction.md`), but still not drawn into the ASCII diagram above — sequenced verbally instead: `B0 → B1 → BH → BR → B3 → B4 → B6-Sys` for Stage 1 (the hub-and-raid structural loop has to exist before persistence/world/progression phases can be fully scoped around it), and `B4X → BF → B5` for Stage 2 (heist-style contracts in B5 need `BF`'s hostile faction to guard the loot they gate). Redraw this ASCII chain for real once all three have had at least one real implementation pass against their new files, same as `BV`'s dependency edges below.
...
| **`BH`/`BR`/`BF` (CR-13's new phases) have a task breakdown now (2026-08-27) but no real session-estimate history yet** | **MEDIUM** | Downgraded from MEDIUM-HIGH now that each has entry/exit criteria and a task table — same discipline `B4` and `BV` each got before their own scoping pass. Re-forecast the size estimates after the first real multi-session task in each (`BH-T2`, `BR-T3`, `BF-T2`) lands, per `02_MasterWorkflow.md` §3 Step 6 — don't let the placeholder sizes harden into a commitment before then. `BH`/`BR` still gate Stage 1's exit criteria (the hub-and-raid loop *is* the core loop now). |
