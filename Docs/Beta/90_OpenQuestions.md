...
### OQ-BH-01 — Walkable hub or menu-driven screen flow? ⚠️ SUPERSEDED 2026-08-29 (see CR-14) — was ✅ RESOLVED 2026-08-28
**Original 2026-08-28 answer (no longer current):** menu-driven screens, no hub level content.
**New answer, 2026-08-29 (dev-confirmed, `00_MasterPlan.md` CR-14):** **walkable, physical hub — Grayback Lodge**, a converted mountain hunting-lodge/outfitter compound. `BH-T6.1` (the walkable-hub-level path) is un-cut and becomes the real scope; `T6.1-alt`'s menu-flow becomes the one that's cut instead. Stash/vendor/contract screens still reuse B1's `UZSUserWidgetBase`/`UZSUIManager` modal stack — they just open via a physical interactable (walk up, interact) the same way `AZSContainerActor`/`UZSContainerLootWidget` already work, not via hub-wide menu navigation. See `BH_HubHideoutEconomy.md`'s rewritten hub description for the compound's structures (Motor Barn, stash vault, armory, lookout tower).

### OQ-BH-02 — Per-player or shared stash in a hosted co-op world? ✅ RESOLVED 2026-08-28 (dev-confirmed)
**Per-player stash.** Each connected client gets its own separate stash/currency, not one shared pool per game instance. Real engineering work still needed: `UZSHubSubsystem`'s `Currency`/`Stash` need to key off the requesting player (`PlayerState` or equivalent) instead of being one flat `GameInstanceSubsystem`-wide pool — not built yet, tracked as the concrete next step on `BH-T1`. `BH-T3.2`'s contract-tracking scope question ("scoped per the same per-player-vs-shared question") is resolved the same way: per-player. **Unaffected by CR-14** — `Stash`/`Currency` already live on `AZSPlayerState`, which stays correct whether the hub is a menu or a physical space.
...
### OQ-BH-04 — Hub theme and key structures ✅ RESOLVED 2026-08-29 (dev-confirmed, CR-14)
**Grayback Lodge** — a converted big-game hunting lodge/outfitter compound on a timbered ridge, seized as a mercenary forward operating base. Key structures: the **Lodge** (vendor/quartermaster + contract board, taxidermy/gun-rack decor doubling as the armory backdrop), the **Cache** (a root-cellar/walk-in-cooler style stash vault), the **Motor Barn** (the old equipment barn, now storage for recovered/drivable vehicles — see `OQ-BR-06`), and a **ridge lookout tower** (overwatch flavor now, a natural home for a future scouting/recon mechanic later, not scoped yet). Feeds `BH-T6.1`'s level content and `BH-T2`'s vendor NPC naming/flavor.

---

## BR — Raid Lifecycle & Extraction *(new, CR-13, extraction pivot 2026-08-27)*

### OQ-BR-01 — Can one player leave a shared raid without ending it for teammates? ⚠️ MOOT 2026-08-29 (see CR-14) — was ✅ RESOLVED 2026-08-28
**No longer applicable.** This question assumed "raid" was a bounded, shared session someone could leave early — CR-14 retires that concept entirely (see `OQ-BR-03` below). With one persistent zone and fully independent player movement (`OQ-BR-05`), there's no session for anyone to "leave without ending it," because there's no session to end. The spectate-until-raid-ends mechanic this question's 2026-08-28 answer produced (`AZSPlayerCharacter::Server_EnterSpectatorAfterDeath`, `AZSGameState::IsRaidOver`/`Server_CheckRaidEndAndReset`) is now believed obsolete — not yet removed from code, flagged for the implementation pass that follows this design record.

### OQ-BR-02 — Should extraction have a delay/channel time? 🟡
**Still open, but likely moot pending `OQ-BR-04`'s flagged assumption** — if the extraction-point ceremony is retired in favor of an ordinary stash-deposit interaction (see `OQ-BR-04` below), there's no discrete "extract" action left to gate with a channel time. Revisit once `OQ-BR-04` is confirmed either way.

### OQ-BR-03 — Does entering the zone reload the level, or does it stay persistently loaded? ⚠️ SUPERSEDED 2026-08-29 (see CR-14) — was ✅ RESOLVED 2026-08-28
**Original 2026-08-28 answer (no longer current):** level reload each raid entry.
**New answer, 2026-08-29 (dev-confirmed, CR-14):** **persistently loaded — one continuous, never-reloaded world.** The hub (Grayback Lodge) and the quarantine zone are the same persistent space; players drive or walk between them with no load boundary. Consequence: the loot-reseed-via-reload mechanism this question's old answer relied on is gone — replaced by `OQ-BR-05`'s spawn-director/horde system for zombies and a **deliberately non-replenishing** loot economy (`OQ-BR-05`) rather than any reseed at all. The dropped-loot-persistence need this question originally flagged (pulling a slice of `B3` forward) is unchanged in spirit but simpler now: there's no reload to survive across, loot just stays where it fell in the one persistent world, same as it always would in a classic persistent-open-world game.

### OQ-BR-04 — Does an "extraction point" ceremony still exist, now that there's no raid to extract from? 🟡 flagged, not yet dev-confirmed
**Rec (my inference from CR-14's other five answers, not independently asked): no.** Carried loot becomes safe once physically deposited into the stash at the compound — an ordinary container-deposit interaction (reusing the existing `Server_StoreInBag`/stash-deposit machinery), not a special "extract" action with its own actor/interactable/channel-time question (`OQ-BR-02`). `AZSExtractionPointActor` and `Server_RequestExtraction`'s ceremonial banking step would be retired under this reading, not ported forward. **Needs explicit confirmation before either is removed from code** — flagged exactly like `GameDevPlan.md`'s existing "recommended, not yet dev-confirmed" convention for the investigation-arc fold-in.

### OQ-BR-05 — How does the world stay dangerous/interesting without a per-raid reseed? ✅ RESOLVED 2026-08-29 (dev-confirmed, CR-14)
**Two different answers for two different resources, deliberately not symmetric.** Zombies: a **spawn-director** periodically fills zombies back into map regions the players aren't currently observing (avoids visible pop-in), plus **hordes periodically migrate through the map** as a standalone roaming threat independent of player position — closer to a living population than a fixed count. Loot: the opposite — the world seeds with a **substantial** amount of items up front, but **containers do not replenish once emptied**, a fully finite/consumable economy that pushes players to range further out over time rather than farm one area forever. Reframes most of `BR-T3`'s original "raid reseed" task line — see `BR_RaidLifecycleExtraction.md`'s new pivot notice.

### OQ-BR-06 — How far do vehicles go as part of this pivot? ✅ RESOLVED 2026-08-29 (dev-confirmed, CR-14)
**Basic drivable transport now; full depth stays deferred to `BV`.** Real driving controls and entering/exiting seats, in and out of the compound freely, land as part of this pivot. Fuel, damage, combat (running over zombies, drive-by), and proper storage/loadout integration stay with `BV`'s own dedicated scoping pass (`CR-02`), unscheduled until Stage 1's core loop is done — this is a deliberate scope split, not a decision to do `BV` in full now.
