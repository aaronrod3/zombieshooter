...
| T11.2 | 🔧 **Code complete 2026-07-26, not yet PIE-verified.** New `SecondaryAction` input (bound `T`, resolving OQ-B0-10) → `HandleSecondaryAction` → `Server_HandleSecondaryAction`, dispatching on `SecondaryHandInstanceId`'s resolved config. **Partial**: the toggleable-item branch (T11.3/T11.4) is fully implemented; the weapon-attack branch (reusing `PerformMeleeSwing`/`Server_Fire`) is a documented content gap - see T11.4's note. | OQ-B0-10 |
...
| T11.4 | 🔧 **Code complete 2026-07-26, not yet PIE-verified.** `OnSecondaryItemToggled`'s default implementation toggles a new `AZSPlayerCharacter::FlashlightComponent` (a real `USpotLightComponent`, always present on the character, not a Blueprint-authored cosmetic) - toggling `IA_SecondaryAction` over a `bIsToggleable` item genuinely turns a light on/off in PIE today, the thing B4's darkness mechanic will be designed against. **Content gaps**: no real `DA_ZS_ItemConfig_Flashlight` instance authored yet (`bIsToggleable = true`, otherwise a normal item); `FlashlightComponent`'s attach offset is a rough guess (no confirmed left-hand socket exists on the skeleton) rather than a real socket - positioning/mesh are content-polish, not blocking the mechanism. **Scope note carried from T11.2**: an offhand *weapon* (a pistol placed in SecondaryHand) doesn't fire/swing yet - `Docs/Planning/InventoryLoadoutEquipping_Plan.md` §6 itself calls this "genuinely new surface, not just wiring... scope it as its own small task," so it's flagged here as a deliberate, documented gap rather than attempted half-built (it would need its own spawned `AZSWeapon` actor and ammo/equip choreography mirroring `CurrentWeapon`'s). | X-2 |
