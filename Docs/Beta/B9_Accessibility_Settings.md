...
| T3.3 | **Gamepad support pass — this is where ALL gamepad work happens.** Per OQ-B9-01 (resolved 2026-07-23), every phase before this one deliberately deferred gamepad verification; nothing has been tested on a controller since P1, and P1's claim was never evidenced. Assume it is broken until proven otherwise. The design groundwork exists — the top-down twin-stick scheme was chosen partly for gamepad-friendliness (`GameDevPlan` §1) — but **budget this as build-and-verify, not check-and-confirm.** ⚑ **Hotbar access needs its own gamepad answer, not just a mapping exercise**: B0-T3.4 (OQ-B0-01) removed hotbar cycling entirely — it's 1-9 direct-select only now, scroll wheel is camera zoom, there is no `IA_HotbarCycle` to map to a bumper/d-pad. Decide the gamepad-hotbar mechanism here (radial select, d-pad + face-button combo, or something else) as a real design task, not an assumed one. |
...
| **PT2** | End of T3 | **Gamepad-only session — the first real controller test since P1.** Play 30 minutes with the keyboard physically unplugged, including inventory, container looting, and the map. | Every screen is navigable. Combat feels controllable. All 9 hotbar slots are reachable via whatever mechanism T3.3 settled on. Expect to find real problems here; nothing has been controller-tested for the entire project. |
