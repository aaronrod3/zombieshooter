...
| # | Revision | Src | Impact | B0 task |
|---|---|---|---|---|
| P4-R1 | **Add search-last-known-location** to the BT. Full loop: wander → investigate noise → chase → **search last location** → resume wander. | `[C]` §6 | ✅ **Mostly done, 2026-07-25.** The loop's "chase → search-last-location" half already worked (`MoveTo(LastKnownLocation)`); the "resume wander" half was structurally broken (`BTTask_StartInvestigationTimer` returned `InProgress` forever, permanently blocking `Wander` as its Sequence sibling) — fixed as part of the native BT task migration (all 6 tasks ported to C++, see `Docs/SessionHandoff.md`). `BTTask_ClearLastKnownLocation` (the "give up after N seconds" piece) exists as a native class now but is still **not wired into the tree** — that specific wiring decision is genuinely ambiguous (it may overlap with `StartInvestigationTimer`'s own expiry-driven clear) and stays open, folded into the trimmed OQ-B4-12. | OQ-B4-12 (ClearLastKnownLocation wiring only) |
...
| P4-R3 | **`BT_Zombie`'s wander branch has zero children** — zombies have no idle behaviour at all. | `[N]` handoff | ✅ **Fixed 2026-07-25.** Two separate problems, both resolved: (1) the two stale class references were repointed (2026-07-24 hygiene pass); (2) that alone didn't create ambient wander — the existing `Wander` node was reachable only from *inside* the Investigate branch (gated on `LastKnownLocation` being set), so a zombie that had never perceived anything had no branch to take at all. Fixed by adding a genuine undecorated, lowest-priority `Wander` branch directly on the root selector. **Behaviorally unverified** — blocked on a navmesh issue in the test level, dev fixing manually. | B0-T8 (done, pending PIE verification) |
