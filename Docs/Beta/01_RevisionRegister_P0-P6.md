...
| # | Revision | Src | Impact | B0 task |
|---|---|---|---|---|
| P4-R1 | **Add search-last-known-location** to the BT. Full loop: wander → investigate noise → chase → **search last location** → resume wander. | `[C]` §6 | `LastKnownLocation` Blackboard key already exists and is written by `HandleTargetPerceptionUpdated`; the BT branch consuming it does not. | B0-T8 |
| P4-R2 | **Confirm senses stay fixed per TYPE, never randomized per-individual.** | `[C]` §6 | Already true — `UZSZombieConfig` radii are read at `OnPossess`. **Verify no randomization creeps in later**; record as a standing rule. | B0-T8 |
| P4-R3 | **`BT_Zombie`'s wander branch has zero children** — zombies have no idle behaviour at all. | `[N]` handoff | The base state of the game's only enemy is unimplemented. High player-visible impact. | B0-T8 |
