...
- **BH** — Scoped (`BH_HubHideoutEconomy.md`, 2026-08-27). Both `OQ-BH-01`/`OQ-BH-02` resolved 2026-08-28. `BH-T1`'s backend now lives on `AZSPlayerState` (per-player, actually replicated - moved off `UZSHubSubsystem`, which was per-game-instance and never replicated to a non-host client at all) and is built/tested. Next real move: `BH-T2`'s remaining pieces and `BH-T5`'s UI, now safe to build against a real backend.
