...
## Tasks
- [x] Finish Phase 3 M7 — 2-client PIE verification of the pre-pivot replication layer. **Dev-confirmed 2026-07-20: everything works and replicates correctly except crouch** - and crouch's failure is the already-separately-tracked AnimGraph pose bug (`Docs/SessionHandoff.md`), not a new replication finding. The replication layer itself checks out.
...
## Exit criteria
Clean build passes (confirmed). 2-client PIE verification (M7) confirmed (crouch pose bug excepted, tracked separately). This phase's exit criteria are met.
